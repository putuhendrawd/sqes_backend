...
def mock_requests_get(mocker):
    ...
    return mocker.patch("sqes.services.data_updater.requests.get", return_value=mock_response)

@pytest.fixture
def mock_pandas_sql(mocker):
    """Mocks pandas' SQL read and write functions."""
    fake_station_list = pd.DataFrame({'code': ['TEST']})
    mocker.patch("sqes.services.data_updater.pd.read_sql", return_value=fake_station_list)
    
    mock_to_sql = mocker.patch("sqes.services.data_updater.pd.DataFrame.to_sql")
    return mock_to_sql
...
def mock_pandas_concat(mocker):
    ...
    mocker.patch("sqes.services.data_updater.pd.concat", mock_concat)
    return mock_concat
...
def mock_sqlalchemy(mocker):
    ...
    mocker.patch("sqes.services.data_updater.create_engine", return_value=mock_engine)
...
def test_data_updater_full_run(
    ...
    data_updater.update_sensor_table(
        db_type='postgresql',
        db_creds=db_creds,
